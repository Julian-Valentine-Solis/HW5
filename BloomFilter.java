...
class BloomFilter {
    ...
    public boolean contains(String s) {
        ...
        // Loop over the entirety of the number of hashes to create various hash codes
        // for the given element
        for (int i = 0; i < noHashes; i++) {
            // use hashCode method to produce hashCode
            int indexOfBit = (int) ((hashCode(s, i)) & hashMask);// Ensure that the hashCode created is
            // within bounds

            if (!data.get(indexOfBit)) {// if any bit is not in the set, then s is not in the set
                return false;
            }
